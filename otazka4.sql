...
--krok 1: vytvoření dočasných tabulek 

CREATE TEMP TABLE price_change AS (SELECT date_part ('year', DATE_FROM ) AS food_price_year, tb1.FOOD_CODE , tb1.FOOD_NAME , AVG(tb1.FOOD_PRICE) AS avg_food_price,
...
--tyto tabulky použiji v dalším kroku
...
--kontrolní dotaz (základní skript, kde se zobrazí data - existuje rok, kde byl nárůst cen vyšší než 10 %)
WITH price_payroll_change AS (
 SELECT *
FROM price_change pc
JOIN payroll_change payc
 ...
 SELECT ppc.food_price_year, ppc.food_name, ppc.price_change/ppc.payroll_change AS price_payroll_rate
FROM price_payroll_change ppc
WHERE ppc.price_change/ppc.payroll_change > 1.1
ORDER BY price_payroll_rate