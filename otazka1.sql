...
WITH payroll_comparison AS
  (SELECT tb1.industry_branch_code, 
          tb1.industry_branch_name, 
          tb1.payroll_year, 
          AVG(value) AS avg_payroll,
          LAG(tb1.payroll_year) OVER (PARTITION BY tb1.industry_branch_code
                                      ORDER BY tb1.PAYROLL_YEAR) previous_payroll_year,
          AVG(value) - lag(AVG(value)) OVER (PARTITION BY tb1.industry_branch_code
                                             ORDER BY tb1.PAYROLL_YEAR) payroll_change
   FROM T_TEREZA_BOHMOVA_PROJECT_SQL_PRIMARY_FINAL tb1
   WHERE tb1.value_type_code = '5958'    --průměrná hrubá mzda na zaměstnance
     AND tb1.calculation_code = '200'   --mzda na přepočtený počet zaměstnanců
   GROUP BY tb1.industry_branch_code, 
            tb1.industry_branch_name,
            tb1.payroll_year)   
